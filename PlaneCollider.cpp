...
Collision PlaneCollider::CollidesWith(AABBCollider& other)
{
	...
	Vector3 aabbHalfExtents = other.GetHalfExtents();
	Vector3 aabbCentre = other.GetCentre();
	std::array<Vector3, 8> aabbVertices = other.GetVertices();
...
Collision PlaneCollider::CollidesWith(SphereCollider& other)
{
    ...
    float distance = (spherePosition * planeNormal) - sphereRadius - planeDistance;
